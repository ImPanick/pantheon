...
| Seventeen checkers in CI, each one built from a defect that actually shipped. | `0` | **`17`** | `counted` |
| Test files 792 -> 938, and a suite of 9,007 passing with nothing red. | `792 test files` | **`938 test files · 9,007`** | `diffed` |
...
### Seventeen checkers in CI, each one built from a defect that actually shipped.

**`0` → `17`**  ·  provenance **`counted`**

**Odysseus:** No repository-level checkers.

**Pantheon:** `.pantheon/release-gate.py` runs all seventeen, reading the list from `ci.yml` rather than keeping a second copy of it.
...
### Test files 792 -> 938, and a suite of 9,007 passing with nothing red.

**`792 test files` → `938 test files · 9,007`**  ·  provenance **`diffed`**
