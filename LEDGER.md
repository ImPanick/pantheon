...
| Twenty-two checkers in CI, each one built from a defect that actually shipped. | `0` | **`22`** | `counted` |
...
### Twenty-two checkers in CI, each one built from a defect that actually shipped.

**`0` → `22`**  ·  provenance **`counted`**

**Odysseus:** No repository-level checkers.

**Pantheon:** `.pantheon/release-gate.py` runs all twenty-two, reading the list from `ci.yml` rather than keeping a second copy of it.
