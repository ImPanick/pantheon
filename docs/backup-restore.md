...
Pantheon keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/pantheon-backup` tool
...
```bash
# Create a snapshot → backups/pantheon-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/pantheon-backup snapshot

# List existing snapshots (most recent first)
./scripts/pantheon-backup list

# Check a tarball's integrity without extracting it
./scripts/pantheon-backup verify backups/pantheon-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/pantheon-backup restore backups/pantheon-backup-20260101-120000.tar.gz --yes
```
...
./scripts/pantheon-backup snapshot --out /mnt/nas/pantheon-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/pantheon-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/pantheon && ./scripts/pantheon-backup snapshot --out "/mnt/nas/pantheon-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `pantheon-backup` therefore does not capture the Docker
