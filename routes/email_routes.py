...
def _parse_list_unsubscribe_header(value: str | None) -> list[dict]:
    ...
    first-pass Pantheon flow. HTTP unsubscribe links are useful evidence but
...
def _apply_pantheon_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-Pantheon-Origin"] = PANTHEON_MAIL_ORIGIN
    if kind:
        msg["X-Pantheon-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-Pantheon-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _fixture_email_record(row: dict, uid_num: int, owner: str) -> dict:
        ...
        return {
            "uid": uid,
            "message_id": f"<fixture-email-{uid}-{owner_key}@fixtures.pantheon.local>",
            "subject": subject,
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the Pantheon marker header, but include the subject
                # fallback too. The fallback uses a distinct Pantheon prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-Pantheon-Kind "reminder" SUBJECT "Reminder (Pantheon):"{from_clause})',
                )
    ...
    def execute_unsubscribe(data: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            msg_out["Message-ID"] = email.utils.make_msgid(domain="pantheon.local")
            _apply_pantheon_headers(msg_out, "unsubscribe", uid)
    ...
    @router.delete("/pantheon/reminders")
    async def delete_pantheon_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as Pantheon reminders."""
        if account_id:
            _assert_owns_account(account_id, owner)
        deleted = 0
        folders_checked = []
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-Pantheon-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (Pantheon):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (Pantheon):")})'))
                            # Legacy reminders created before the Pantheon
            ...
            logger.error(f"delete_pantheon_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    @router.post("/compose-from-pantheon")
    async def compose_from_pantheon(data: dict, owner: str = Depends(require_owner)):
        """Stage an Pantheon document or gallery image as a compose upload."""
        ...
        try:
            ...
            logger.error(f"Failed to stage Pantheon attachment {kind}/{item_id}: {e}")
            return {"success": False, "error": "Mail operation failed"}

    @router.post("/compose-from-pantheon-zip")
    async def compose_from_pantheon_zip(data: dict, owner: str = Depends(require_owner)):
        """Stage several Pantheon documents/gallery images as one zip attachment."""
        ...
        try:
            ...
            logger.error(f"Failed to stage Pantheon zip attachment: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", pantheon_kind=None, pantheon_ref=None,
    ):
        ...
        _apply_pantheon_headers(outer, pantheon_kind or "scheduled", pantheon_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, pantheon_kind, owner)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("pantheon_kind") or "scheduled",
                owner or "",
            ))
            conn.commit()
            conn.close()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="pantheon.local")

        if req.in_reply_to:
            outer["In-Reply-To"] = req.in_reply_to
        if req.references:
            outer["References"] = req.references
        if req.pantheon_kind:
            _apply_pantheon_headers(outer, req.pantheon_kind)
