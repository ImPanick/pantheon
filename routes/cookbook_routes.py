...
_HF_TOKEN_STATUS_SNIPPET = (
    ...
    'Add one in Pantheon Cookbook -> Settings -> HuggingFace Token."; '
    'fi'
)
...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def _repair_cookbook_known_host(host: str, ssh_port: str | None = None) -> tuple[bool, str]:
        """Refresh Pantheon' own known_hosts entry for a validated Cookbook host.
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            if ("llama_cpp" in req.cmd or "llama-server" in req.cmd) and not local_windows_llama_cmd:
                ...
                runner_lines.append('# Auto-generated by Pantheon Cookbook: a `llama-server` lookalike')
                ...
                runner_lines.append('  echo "ERROR: MLX image serving requires FastAPI + uvicorn + python-multipart in the launch Python: $PANTHEON_MLX_IMAGE_CMD_PY. Install the MLX image dependencies in Cookbook Dependencies."')
                runner_lines.append('  PANTHEON_PREFLIGHT_EXIT=127')
                runner_lines.append('fi')
                ...
                runner_lines.append('    echo "Build it from swift/pantheon-mlx-image-bridge in Cookbook Dependencies."')
                runner_lines.append('    PANTHEON_PREFLIGHT_EXIT=127')
                runner_lines.append('  fi')
            ...
            if (not handled_ollama_serve
                ...
                runner_lines.append('  echo "[pantheon] Ollama sidecar model is available; keeping Cookbook task attached to the persistent Ollama daemon."')
