                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Pantheon (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Pantheon" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "pantheon_kind" if "pantheon_kind" in cols else "'scheduled' AS pantheon_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                # externally cron-driven `pantheon-mail poll-scheduled`, or
                # an admin running the CLI manually alongside the in-process
                # one despite the PANTHEON_INPROCESS_POLLERS=0 guidance) -
                ...
                pantheon_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Pantheon-Origin"] = "pantheon-ui"
                outer["X-Pantheon-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", pantheon_kind or "scheduled")[:64]
                outer["X-Pantheon-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `pantheon-mail poll-scheduled` CLI for
...
def _start_poller():
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (PANTHEON_INPROCESS_POLLERS=0); "
            "drive `pantheon-mail poll-scheduled` externally."
        )
        return
    import asyncio
