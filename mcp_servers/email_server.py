...
_OWNER_ENV_KEYS = ("PANTHEON_MCP_EMAIL_OWNER", "PANTHEON_EMAIL_OWNER")
_OWNER_SCOPE_ERROR = (
    "Error: email MCP requires an authenticated owner or PANTHEON_MCP_EMAIL_OWNER "
...
def _default_document_owner() -> str | None:
    ...
    """
    owner = os.environ.get("PANTHEON_DOCUMENT_OWNER", "").strip()
    if owner:
        return owner
    try:
...
def _fixture_account_rows() -> list[dict]:
    ...
    for row in _fixture_email_rows(owner or None):
        email_addr = row.get("account_email") or owner or "fixture@fixtures.pantheon.local"
        if email_addr not in owners:
            owners.append(email_addr)
    if not owners:
        owners = [owner or "fixture@fixtures.pantheon.local"]
    return [
        {
            "id": "fixture-email",
    ...
    try:
        ...
        conn.execute("""
            ...
            (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr,
             attachments, send_at, created_at, status, account_id, pantheon_kind, owner)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'agent_draft', ?, ?, ?)
        """, (
            pending_id,
...
def _draft_reply_to_email(uid, body, folder="INBOX", reply_all=False, account=None, title=None):
    """Create a threaded Pantheon reply draft document. Does not send."""
    conn = _imap_connect(account)
    conn.select(_q(folder), readonly=True)
    status, msg_data = conn.uid("FETCH", _b(uid), "(BODY.PEEK[])")
    conn.logout()
...
async def _ai_draft_reply_to_email(uid, folder="INBOX", reply_all=False, account=None, title=None):
    """Generate a reply with Pantheon' AI-reply prompt/style, then create a compose doc."""
...
async def list_tools() -> list[Tool]:
    ...
    return [
        ...
        Tool(
            name="draft_email",
            description=(
                "Create a new Pantheon email compose draft document. This DOES NOT send. "
                ...
                "can edit or press Send in Pantheon. "
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "title": {"type": "string", "description": "Optional Pantheon document title"},
                    **ACCOUNT_PROP,
                },
        ...
        Tool(
            ...
            description=(
                "Create an Pantheon email reply draft document for an existing email UID. "
        ...
        Tool(
            ...
            description=(
                "Generate an AI reply using Pantheon' existing AI Reply behavior, "
    ...
    try:
        ...
        if name == "list_emails":
            ...
            if result.get("pending"):
                return [TextContent(
                    ...
                    text=(
                        ...
                        "Nothing has been sent yet. Review and approve it in Pantheon before delivery."
                    ),
                )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Created Pantheon email draft `{result['title']}` "
                    f"(document ID: {result['doc_id']}){acct_note}. "
                    "It has not been sent; open the document in Pantheon to review and send."
                ),
            )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Created Pantheon reply draft `{result['title']}` for UID {uid} "
                    f"(document ID: {result['doc_id']}){acct_note}. "
                    "It has not been sent; open the document in Pantheon to review and send."
                ),
            )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Generated AI reply and created Pantheon compose draft "
