...
AREAS = (
    ...
    ("Mailboxes and providers", "Signing in to somebody else's service, which Law 16 permits only because a person asked for it."),
...
CLAIMS: Tuple[Claim, ...] = (
    ...
    Claim(
        ...
        how=(
            ...
            "a run status written as a bare string literal at fifteen sites, an extension-to-language map hand-copied into the browser four times while the server derived it once, thirty-one Python dependencies with no version between them and a Dependabot config that could therefore never bump one. "
            "And a vendored library whose version lived in prose and drifted eighteen releases before anybody looked. "
    ...
    Claim(
        id="tests",
        area="Verification apparatus",
        headline="Test files 792 -> 1,167, and a suite of 13,112 passing with nothing red.",
        stock="792 test files.",
        pantheon="1,167 test files, 13,112 tests passing, 0 failing.",
        before="792 test files",
        after="1,167 test files · 13,112",
        provenance="diffed",
    ...
    Claim(
        id="providers-are-records",
        area="Mailboxes and providers",
        headline="One mail provider, hand-rolled, and Microsoft refused in three places -> a provider is a record, and Microsoft works.",
        stock=(
            "Google, spelled out. Two hand-written routes — `/oauth/google/authorize` "
            "and `/oauth/google/callback` — a hand-built authorize URL, a hand-built "
            "token exchange and a hand-built userinfo fetch, with `\"google\"` written "
            "into `routes/email_routes.py` nine times. **Microsoft appears zero times "
            "in that file and seven times in `routes/email_helpers.py`**, all of them "
            "an error message saying it cannot be used; `static/js/settings.js` says "
            "*does not support Microsoft* twice, and `docs/email-outlook.md` tells the "
            "reader Outlook accounts *cannot currently be added*. The eight integration "
            "presets carry `auth_type` of `bearer`, `header` or `none` only — no client "
            "id, no scope, no endpoint — so they cannot describe a sign-in either."
        ),
        pantheon=(
            "`src/providers.py`: seven records, one type. A mailbox is that record with "
            "a `mail` block and a service is the same record without one, so the mail "
            "flow and the integration presets read the same shape. Google and Microsoft "
            "are linkable mailboxes driven by **one** parameterised route pair; GitHub, "
            "Slack, Anthropic and OpenAI project into `INTEGRATION_PRESETS`, taking it "
            "from 8 presets to 12. The registered callback URL is unchanged, so no "
            "existing deployment's client registration breaks."
        ),
        before="1 mail provider · 2 hand-written routes · 8 presets · Microsoft refused in 3 places",
        after="7 records · 2 linkable mailboxes · 1 route pair · 12 presets · Microsoft supported",
        provenance="diffed",
        repro=(
            f"git show {FORK_POINT}:routes/email_routes.py | grep -c '\"google\"' && "
            f"git show {FORK_POINT}:routes/email_routes.py | grep -n 'router.get(\"/oauth' && "
            f"git show {FORK_POINT}:routes/email_helpers.py | grep -ci microsoft && "
            f"git show {FORK_POINT}:src/integrations.py | grep -cE '^    .[a-z_]+.: [{{]$' && "
            "python3 -c \"from src import providers; print(len(providers.PROVIDERS), providers.mail_provider_ids())\" && "
            "python3 -m pytest tests/test_a_provider_is_a_record.py -q"
        ),
        evidence=(
            "src/providers.py",
            "tests/test_a_provider_is_a_record.py",
            "docs/email-outlook.md",
        ),
        rows=("P18-05", "D-2026-09-12-01"),
        how=(
            "**The proof is not a grep** (`Law 20`). A test invents a provider called "
            "`acme`, inserts the record at runtime and drives the served list, the "
            "authorize redirect, the transport guards and the refresh table — with no "
            "module edited. If any step still asks *which provider is this* by name, "
            "`acme` falls out of that step. "
            "**The second provider is what taught the record its shape**, three times "
            "over: Microsoft has two SMTP hosts against one IMAP host, so a scalar "
            "field could not hold it; Microsoft's verified address cannot come from a "
            "userinfo call, because Entra will not issue one token covering Graph "
            "**and** the `outlook.office.com` resource scopes IMAP and SMTP need, so it "
            "is read from the `id_token` — and that value is the reconnect ownership "
            "check, not a convenience; and the same vendor is sometimes two records, "
            "since OpenAI's platform API is a key while a ChatGPT subscription is a "
            "sign-in. "
            "**What is deliberately not claimed**: Anthropic and OpenAI have no "
            "consumer OAuth for API access. Giving them an `authorize_url` so every row "
            "looked alike would have been a lie told for symmetry, so they are "
            "`api_key` records and the panel offers a key rather than a button that "
            "cannot work. "
            "**Two checkers and a mutation run shaped the result.** "
            "`check-env-declared.py` refused the first design, in which variable names "
            "were composed from a prefix: `MICROSOFT_OAUTH_REDIRECT_URI` was then "
            "declared in `.env.example` and appeared **nowhere in the source**, so an "
            "operator could set it, believe something changed, and have no way to find "
            "out. A mutation run left two rules alive — nothing exercised a refresh "
            "that returns a *new* refresh token, so *Microsoft rotates and Google does "
            "not* was implemented and unproven; and the never-overwrite rule on the "
            "generated presets was asserted against a preset that is not one of the "
            "four ids, so it proved nothing. Both are now tested."
        ),
    ),
    ...
    Claim(
        ...
        headline="Unreachable UI 78 -> 120 -> 25 and ratcheted, with 1,524 lines deleted against 340 added.",
        ...
        # repro carried `--max 124` against CI's ceiling — so a reader
        ...
        #
        # `78 -> 120 -> 25` is not a regression: the `78` was counted before `B58`
        ...
        #
        # `40 -> 25` on 2026-09-18: `P2-19` and `P2-20` wired the fifteen
        # `adm-*` ids that were the largest single group in the list — six
        # webhook, eight RAG and one feature-toggle target, all of them
        # markup that was never written for code that already existed. The
        # ceiling moved down with the count, which is the only direction it
        # is allowed to move.
        after="25",
        provenance="measured",
        repro="python3 .pantheon/check-wiring.py --max 25",
        ...
        how=(
            ...
            "reached: `check-wiring.py` reports **25** unresolved lookups today and "
            "`ci.yml` refuses 26. It said `2` here until `B348`, against its own "
