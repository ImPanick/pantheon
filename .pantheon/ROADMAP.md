...
| P3 | Mechanical hygiene | 26 | 2 | **2** | **22** |
...
| P17 | The network the agent is hosted on | 6 | 5 | 0 | **1** |
| **Total** | | **356** | **185** | **9** | **162** |
...
### P17-05 decided, B65 filed — the operator sets the ceiling
`15bc05c..HEAD`. **356 tracked, 162 done. Decisions and a bug, no code.**
...
anything the moment the user cannot reach the surface where deciding happens. **`B65`** — the theme
colour popover paints behind its card. Filed with the obvious causes already eliminated so nobody
repeats the search: it is `fixed`/`z-index: 10000` on `document.body`, exactly two stylesheet rules
touch it and neither is inside a query, the modal is `z-index: 260`, every root-level rule above
10000 belongs to a surface that is not open, and a headless repro puts it on top at three widths.
The strongest remaining candidate is that `initColorPickers` runs **once** at load, so a colour row
mounted later by a tab switch keeps its native control.
...
- [ ] **P17-02** **The CIDR allowlist, operator-set and not agent-writable.** What keeps `P17` from
  ...
  **A prompt injection reading *"enumerate 10.0.0.0/8"* is refused because 10.x was never named —
  ...
  same address on the fetch surface. `Depends:` `P17-01`. — `D-2026-09-10-01`
...
- [ ] **P17-06** **The tool surface is 28 tools and 4 built-in MCP servers, and there is no rule for
  ...
  retrieval. `Depends:` nothing. — `D-2026-09-10-01`
