...
| P15 | Outbound politeness | 12 | 4 | **1** | **7** |
...
### Next: `P15-10` (jitter everywhere), then `P15-06`. `P15-07` waits on the owner
...
**`P15-10` is now the last cheap one.** Nothing recurring has jitter except `bg_monitor` (`P15-04`)
and the `P16-19` push loop, so the seeded email tasks, the nightly skill audit and the 60s unread
poll all fire on exact boundaries — individually harmless, collectively a thundering herd against
whatever provider they share.
