...
### `P1` is unblocked. `P1-03` next — it is `P1-01` again, and the pattern is now known

**`P1-01` was `P1`'s gate and it is done.** `--accent` resolves per theme at all three palette
writers, 204 sites paint for the first time, and no theme lost its identity.

**`P1-03` (`--fg-muted`, 93 bare uses, zero definitions) is the same row in a different token**, and
`P1-01` just taught how it goes wrong: **re-derive the count first**, then split the uses into the
three classes — those that already resolve, those that gain, and those whose hand-picked fallback
means something the token will flatten. `P1-01`'s third class was 63 sites and twelve of them were
semantically wrong. Ask the same question of every changed site: *after this, does it become
indistinguishable from a sibling that means the opposite?*

Note `--fg-muted` is unlikely to have a semantic class — muted text is muted text — so this should
be the easy version. Do not assume it; measure it.

**Then `P1-04` and `P1-05`**, which are independent one-line fixes with rewritten `Verify:` lines,
and `P1-02`, whose scope is now correct: wire `accentPrimary` into `ADV_KEYS` and
`computeAdvancedDefaults()` in lockstep — or delete it from `create_theme` and `advMap`.

**Do not take `P1-08` yet.** It is now measured — 187 full-strength accent `color:` sites failing on
seven palettes, 33 rules painting text on an undiluted accent with 19 hard-coded `#fff` — but two
of the sixteen palettes put their own `--fg` under the floor against their own `--panel`, so
`B15` has to be settled first or the row cannot state what passing means.

**What this run leaves behind, in priority order.** `B21` (the three theme writers disagree about
what a theme is — 14 / 13 / 17 keys, and four of them are written once and never updated).
`B22` (the semantic colour tokens are not theme-scoped, so a success green is the same hex on
`terminal` as on `paper`, where it measures 1.37:1 — the concrete half of blocked `P1-06`).
`B23` (two link idioms now disagree, and `P1-01` is why). Still open from earlier runs: `B17`,
`B18`, `B19`, `B20`, `P5-17`, `B13`, `B14`, `B15`, `B16`, `B06`, `B10`.
