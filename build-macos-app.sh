...
# Build a downloadable macOS launcher app + .dmg for Pantheon.
#
#   ./build-macos-app.sh
#
# Produces:
#   dist/Pantheon.app   — double-click: starts the local server (using this
#                         repo's venv) and opens the UI in an app-style window.
#   dist/Pantheon.dmg   — drag-to-Applications disk image (the downloadable).
#
...
# so rebuild if you move the repo. Override the port with PANTHEON_PORT.
set -e

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_NAME="Pantheon"
INSTALL_DIR="$REPO_DIR"
PORT="${PANTHEON_PORT:-7860}"
DIST="$REPO_DIR/dist"
...
<dict>
    ...
    <key>CFBundleIdentifier</key>      <string>com.pantheon.launcher</string>
    <key>CFBundleVersion</key>         <string>1.0</string>
    <key>CFBundleShortVersionString</key><string>1.0</string>
    <key>CFBundlePackageType</key>     <string>APPL</string>
    <key>CFBundleExecutable</key>      <string>$APP_NAME</string>
    <key>CFBundleIconFile</key>        <string>pantheon</string>
...
#!/bin/bash
# Pantheon.app — start the local server and open the UI in an app window.
...
LOG="$INSTALL_DIR/logs/pantheon-app.log"

notify() { /usr/bin/osascript -e "display notification \"$1\" with title \"Pantheon\"" >/dev/null 2>&1; }
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"Pantheon\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "Pantheon isn't set up yet. Open Terminal and run:
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "Pantheon failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "Pantheon is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Install:       open '$DIST/$APP_NAME.dmg'  (drag Pantheon to Applications)"
