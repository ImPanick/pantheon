...
USER = os.getenv("DEMO_IMAP_USER", "demo@pantheon.local")
...
LOOKUP_MSGID = "<demo-lookup-deepseek@pantheon.local>"
...
def _msg(*, frm, to=None, subject, text, html=None, days_ago=0, hours_ago=0,
    ...
    m["Message-ID"] = msg_id or make_msgid(domain="pantheon.local")
    if in_reply_to:
...
def build_dataset() -> list[dict]:
    ...
    add("INBOX", "", _msg(
        ...
        text=("Hey,\n\nSaw your work on the Pantheon stack — seriously impressive. "
        ...
        html=("<p>Hey,</p><p>Saw your work on the <b>Pantheon</b> stack — seriously "
    ...
    add("INBOX", "", _msg(
        frm="Greg <greg@pantheon-demo.example>",
    ...
    orig_id = make_msgid(domain="pantheon.local")
    add("Sent", "(\\Seen)", _msg(
    ...
    add("INBOX", "(\\Flagged)", _msg(
        frm="Ops Bot <ops@pantheon-demo.example>",
        subject="[URGENT] prod is on fire 🔥 — pantheon-ui 502s",
        days_ago=0, hours_ago=1,
        text=("PAGE: pantheon-ui is returning 502s on the /api/chat endpoint.\n"
...
def _seed_cache() -> None:
    ...
    reply = (
        ...
        "— drafted for you by your Pantheon assistant"
    )
    ...
    try:
        ...
        con.execute(
            ...
            (LOOKUP_MSGID, "", "INBOX", "quick q for the slide — DeepSeek-V3 param count?",
             "greg@pantheon-demo.example", summary, "demo", now))
        con.commit()
...
def main() -> int:
    ...
    try:
        ...
        print(f"seeded {len(items)} demo message(s) into {USER} "
              f"(INBOX + Sent). Switch to the 'Demo' account in Pantheon to view.")
        return 0
    finally:
        try:
            conn.logout()
