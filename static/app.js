...
import chatModule from './js/chat.js?v=20260829trustladder1';
import compareModule from './js/compare/index.js?v=20260829trustladder1';
...
function _submitMobileQueuedInput(input) {
  ...
  window.__pantheonQueueStreamingSubmit = now;
...
try {
  const cachedDefaultChat = JSON.parse(localStorage.getItem('pantheon-default-chat-cache') || 'null');
  if (cachedDefaultChat && cachedDefaultChat.endpoint_url && cachedDefaultChat.model) {
    _defaultChat = cachedDefaultChat;
    window.__pantheonDefaultChat = cachedDefaultChat;
  }
} catch (_) {}
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('pantheon-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initPlanToggle() {
    ...
    window.__pantheonSetPlanMode = (active) => setPlanMode(active, { silent: true });
    const statusToggle = el('plan-mode-status-toggle');
    if (btn) {
      btn.addEventListener('click', () => {
        const st = loadToggleState();
    ...
    if (chatBar && !chatBar._pantheonPlanSwipeToggle) {
      chatBar._pantheonPlanSwipeToggle = true;
      let touchStartX = 0;
      let touchStartY = 0;
      let touchStartAt = 0;
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      if (chk.checked) {
        ...
        if (typeof window.__pantheonSetChatMode === 'function') {
          window.__pantheonSetChatMode(_restoreMode === 'chat' ? 'chat' : 'agent');
        }
  ...
  const UI_VIS_KEY = 'pantheon-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'pantheon-toolbar-visibility';
    try {
  ...
  if (textarea) {
    ...
    window.addEventListener('pantheon:chat-busy-change', () => _syncMobileEnterKeyHint(textarea));
      ...
      if (_shouldQueueFromMobileEnter(e, textarea) || (e.key === 'Enter' && !e.shiftKey && !e.isComposing && !isMobile)) {
        ...
        if (form) {
          if (_isForegroundChatBusy() && textarea.value && textarea.value.trim()) {
            ...
            window.__pantheonQueueStreamingSubmit = Date.now();
          }
          _submitChatFormDirect(form);
        }
      }
...
function startPantheonApp() {
  ...
  if (window.__pantheonAppStarted) return;
  window.__pantheonAppStarted = true;
  const _bumpChatPriority = (ms = 10000) => {
    try {
      window.__pantheonChatBusyUntil = Math.max(window.__pantheonChatBusyUntil || 0, Date.now() + ms);
    } catch (_) {}
  };
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('pantheon-doc-open-' + _curSession) === '1') {
  ...
  window._pantheonBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startPantheonApp, { once: true });
} else {
  startPantheonApp();
}
