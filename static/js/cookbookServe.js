...
import { modelColor } from './chatRenderer.js?v=20260918surfacehalves1';
...
function _formatServeCmdPreview(cmd) {
  ...
  if (mlxDeepSeekV4Compat) {
    ...
    if (homeMatch && shortName) {
      const shimPath = `${homeMatch[1]}/.cache/pantheon/mlx-shims/${shortName}`;
      raw = raw.replace(
  ...
  if (mlxDeepSeekV4Compat) {
    return [
      '# Pantheon runtime compatibility: using sanitized MLX DeepSeek-V4 shim.',
      formatted,
    ].join('\n');
  }
  return formatted;
...
function _rerenderCachedModels() {
  ...
  list.querySelectorAll('.memory-item[data-repo]').forEach(item => {
    item.addEventListener('click', async (e) => {
      ...
      panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="prefix_cache"${sv('prefix_cache',false)?' checked':''} /> Prefix / Radix Cache${_h('vLLM: prefix caching. SGLang: RadixAttention prefix cache; when off Pantheon adds --disable-radix-cache.')}</label>`;
      ...
      panelHtml += `<button class="cookbook-btn hwfit-serve-schedule-arrow" type="button" aria-haspopup="menu" aria-label="More launch actions" title="More launch actions">${chevronIcon()}</button>`;
      panelHtml += `</span>`;
      panelHtml += `</div>`;
      panelHtml += `</div>`;
      ...
      // reflects the stored presets. Standard Pantheon .dropdown look, positioned
