  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('pantheon-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('pantheon-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('pantheon-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('pantheon-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('pantheon-char-sessions', JSON.stringify(charSessions));
...
function _reloadMemoryList() {
  import('./memory.js?v=20260722memoryloading1').then(m => {
