...
function initAppearance() {
  ...
  modalEl.querySelectorAll('[data-privacy-key]').forEach(function(chk) {
    chk.addEventListener('change', function() {
      ...
      localStorage.setItem('pantheon-sensitive-blur', chk.checked ? 'on' : 'off');
      window.dispatchEvent(new CustomEvent('pantheon-sensitive-blur-change', {
...
function syncPrivacyCheckboxes() {
  modalEl.querySelectorAll('[data-privacy-key="sensitive-blur"]').forEach(function(chk) {
    chk.checked = localStorage.getItem('pantheon-sensitive-blur') === 'on';
  });
}
...
function initAccount() {
  ...
  if (logoutBtn) {
    ...
    logoutBtn.addEventListener('click', async () => {
      ...
      // Keep "pantheon-last-user" so the login form remembers the username
      ...
      try {
        const _keepKeys = new Set(['pantheon-last-user']);
        const _toRemove = [];
...
function notifyIntegrationsChanged() {
  try {
    window.dispatchEvent(new CustomEvent('pantheon-integrations-changed'));
  } catch (_) {}
}
...
async function initReminderSettings() {
  ...
  const CHANNEL_HINTS = {
    browser: 'Reminders appear as browser notifications inside Pantheon.',
  ...
  if (!channelSel.dataset.integrationRefreshWired) {
    ...
    window.addEventListener('pantheon-integrations-changed', () => {
  ...
  channelSel.addEventListener('change', () => {
    ...
    try { window.dispatchEvent(new CustomEvent('pantheon-reminder-channel-changed', { detail: { channel: channelSel.value } })); } catch (_) {}
  });
  if (emailToIn) {
    let emailDebounce;
...
async function initEmailAccountsSettings() {
  ...
  // `B69`. Roughly 370 lines lived here: a complete second email-account form,
  // reaching `set-email-accounts-list`, `-form`, `-msg` and `-add-btn`. None of
  // those four ids exists in `static/index.html`, and nothing creates them — so
  // `el()` returned `null`, the guard below returned, and the whole block did
  // nothing on every page load since the fork.
  //
  // **It arrived dead.** Upstream `ea2778d9` — *"Move email account management
  // to integrations"* — removed the markup and left the JavaScript; at the fork
  // point `b4d1293` the markup count is already 0 and the reference count is 1.
  // A migration that moved the hard part and stopped before the last step,
  // which is the pattern this fork's README is about.
  //
  // **The cost was measured, not argued.** `P18-01` and the first pass of
  // `P18-07` both landed in it, with tests passing against code the browser
  // never ran, and `static/js/ui.js` and `static/js/settings/lifecycle.js` had
  // each grown a `'#unified-intg-form, #set-email-accounts-form'` fallback —
  // the shape of code written around a corpse.
  //
  // The live form is `showEmailForm` in the integrations panel. The three
  // buttons above this comment are wired and stay; only the unreachable half
  // is gone.
}
...
async function initEmailSettings() {
  ...
  const styleKey = () => {
    const account = String(window.__pantheonActiveEmailAccount || '').trim();
    return account ? `pantheon-email-writing-style:${account}` : 'pantheon-email-writing-style';
  };
...
const AGENT_CONFIGS = {
  codex: {
    ...
    buildSetup: (origin, token) => `export PANTHEON_URL=${origin}
export PANTHEON_API_TOKEN='${token}'
mkdir -p ~/plugins
curl -fsSL -H "Authorization: Bearer $PANTHEON_API_TOKEN" "$PANTHEON_URL/api/codex/plugin.zip" -o /tmp/pantheon-codex-plugin.zip
python3 -m zipfile -e /tmp/pantheon-codex-plugin.zip ~/plugins
python3 - <<'PY'
import json
...
entry = {
    "name": "pantheon",
    "source": {"source": "local", "path": "./plugins/pantheon"},
    "policy": {"installation": "AVAILABLE", "authentication": "ON_INSTALL"},
    "category": "Productivity",
}
data["plugins"] = [item for item in plugins if item.get("name") != "pantheon"] + [entry]
p.write_text(json.dumps(data, indent=2) + "\\n")
PY
codex plugin add pantheon@personal
python3 ~/plugins/pantheon/scripts/pantheon_api.py capabilities`,
  },
  claude: {
    label: 'Claude Agent',
    word: 'Claude',
...
python3 ~/.claude/skills/pantheon/scripts/pantheon_api.py capabilities`,
  },
};
...
async function initUnifiedIntegrations() {
  ...
  async function showApiForm(editId) {
    ...
    const _applyPreset = () => {
      ...
      if (ntfyHint) {
        ...
        if (isNtfy) {
          ntfyHint.innerHTML = 'Enter the ntfy server URL Pantheon can reach. Examples: <code>http://127.0.0.1:8091</code>, <code>http://100.x.y.z:8091</code>, or <code>https://ntfy.example.com</code>.';
        }
      }
      if (url) {
  ...
  async function showCardDavForm() {
    ...
    const _downloadContacts = async (format) => {
      ...
      try {
        ...
        a.href = url;
        a.download = format === 'csv' ? 'pantheon-contacts.csv' : 'pantheon-contacts.vcf';
  ...
  async function showEmailForm(editId) {
    ...
    // was not the one being fixed: the same work first landed in a second,
    // unmounted copy that `B69` has since deleted.
    //
        ...
        <div class="settings-col">
          ...
          <div id="uf-codex-reveal" style="display:none;width:100%;box-sizing:border-box;">
            ...
            <div id="uf-codex-config-body" style="display:none;">
              <div style="font-size:11px;opacity:0.62;margin:4px 0 6px;">Toggle which Pantheon tools this agent can use. New agents start with chat only.</div>
