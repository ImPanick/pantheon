  ...
  try {
    ...
    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      ...
      if (pkg.installed && pkg.pip_update_available === false && pkg.name !== 'llama_cpp') {
        const tip = esc(pkg.update_note || pkg.status_note || 'Found externally; update outside Pantheon.');
    ...
    function _recipePanelHtml(backend) {
      ...
      return `<div class="cookbook-dep-recipe-panel" data-dep-recipe-panel="${esc(backend)}" data-dep-recipe-active-variant="${esc(initialVariant)}" style="display:none;margin:-4px 0 8px;padding:8px 12px 10px;background:rgba(0,0,0,0.04);border:1px solid var(--border);border-top:none;border-radius:0 0 6px 6px;">
          ...
          <div style="display:flex;gap:6px;justify-content:flex-end;margin-top:6px;">
            <button type="button" class="cookbook-dep-tag cookbook-dep-install cookbook-dep-recipe-run" data-dep-recipe-run="${esc(backend)}" style="display:inline-flex;align-items:center;gap:4px;cursor:pointer;">${playIcon({ size: 11 })}Run</button>
          </div>
        </div>`;
    }
    ...
    const _appDepsHtml = (items) => {
      ...
      const parts = [_sectionHeader('Pantheon app', 'Run inside the Pantheon app itself.')];
      const order = ['System', 'Tools', 'Image', 'LLM', 'Audio', 'Other'];
      for (const cat of order) {
        const catItems = _sortDeps(byCat.get(cat) || [], cat);
        if (!catItems.length) continue;
  ...
  if (dlBtn && dlInput) {
    ...
    const triggerDownload = async () => {
      ...
      if (looksGgufRepo && !pickerInclude) {
        ...
        if (!pickerInclude) {
          uiModule.showToast('Pick a GGUF quant first. Pantheon will not download the whole GGUF repo without an include pattern.');
          return;
        }
...
function _renderRecipes() {
  ...
  html += '</select>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Optional packages that extend Pantheon capabilities.</p>';
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure SSH servers, install Pantheon keys, choose model directories, and set the default server. Local is this machine.</p>';
...
const SHARED_STATE_LEADER_KEY = 'pantheon-cookbook-shared-state-leader';
