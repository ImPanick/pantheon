...
let _searchQuery = '';
let _viewMode = (typeof localStorage !== 'undefined' && localStorage.getItem('pantheon-notes-view')) || 'list'; // 'list' or 'grid'
...
const REMINDER_FIRED_KEY = 'pantheon-notes-reminder-fired';
...
const REMINDER_GLOWED_KEY = 'pantheon-notes-reminder-glowed';
...
const REMINDER_PENDING_HIGHLIGHT_KEY = 'pantheon-notes-reminder-pending-highlight';
const REMINDER_ACTIVE_HIGHLIGHT_KEY = 'pantheon-notes-reminder-active-highlight';
...
const REMINDER_DISMISSED_AT_KEY = 'pantheon-notes-reminder-dismissed-at';
...
function _bringNotesToFront(pane = document.getElementById('notes-pane')) {
  ...
  try {
    window.dispatchEvent(new CustomEvent('pantheon:modal-opened', {
...
export function openPanel() {
  ...
  if (viewBtn) {
    ...
    viewBtn.addEventListener('click', () => {
      ...
      try { localStorage.setItem('pantheon-notes-view', _viewMode); } catch {}
      pane.classList.toggle('notes-view-grid', _viewMode === 'grid');
      _setViewLabel();
      requestAnimationFrame(() => _applyMasonry(document.querySelector('#notes-pane .notes-pane-body')));
    });
...
const _DRAFT_PREFIX = 'pantheon-note-draft-';
