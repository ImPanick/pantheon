// Pantheon UI — Initialization Scripts
...
function markComposerUserEdited() {
  ...
  msgInput.addEventListener('input', () => {
    window.__pantheonComposerUserEdited = !!msgInput.value;
  });
}
...
function clearFreshComposerRestore() {
  ...
  if (window.__pantheonComposerUserEdited || document.activeElement === msgInput) return;
  if (msgInput.value) {
    msgInput.value = '';
...
(async () => {
  try {
    ...
    if (!liveUser) return;
    const KEY = 'pantheon-auth-user';
    const cachedUser = localStorage.getItem(KEY);
    if (cachedUser && cachedUser !== liveUser) {
      const _keepKeys = new Set(['pantheon-last-user', KEY]);
      const toRemove = [];
