...
const _RESEARCH_HINTS = [
  "e.g. Trace Pantheon's ten-year journey home from Troy — every island, monster, and detour, and why each one cost him",
...
const _vizCollapseIcon = chevronIcon({ direction: 'up', size: 12 });
const _vizExpandIcon = chevronIcon({ size: 12 });
...
const _SETTINGS_KEY = 'pantheon-research-settings';
const _COLLAPSE_KEY = 'pantheon-research-settings-collapsed';
...
const _closeIcon = chevronIcon({ size: 18 });
const _playIcon = playIcon({ size: 14 });
...
const _chevronIcon = chevronIcon();
...
function _wireEvents(pane) {
  ...
  pane.querySelector('#research-settings-toggle').addEventListener('click', () => {
    ...
    try { localStorage.setItem('pantheon-research-settings-collapsed', _settingsCollapsed ? '1' : '0'); } catch {}
  });
