...
const _COST_KEY = 'pan-session-cost';
const _COST_RUNS_KEY = 'pan-session-cost-runs';
const _MAX_COST_RUNS_PER_SESSION = 256;
const _COST_LEDGER_LOCK = 'pantheon-session-cost-ledger';
...
const _ACTION_RECENTS_KEY = 'pantheon-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'pantheon-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Pantheon';
      ...
      row.addEventListener('click', () => {
        if (isToolApproval) {
          ...
          if (onSubmit) {
            ...
            card.remove();
            document.dispatchEvent(new CustomEvent('pantheon:tool-approval', { detail }));
          }
        } else {
          send(label);
        }
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Pantheon' : modelRouteLabel(
