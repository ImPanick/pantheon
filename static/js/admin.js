...
async function _selectAddedModelInChat(endpoint) {
  ...
  try {
    document.dispatchEvent(new CustomEvent('pantheon:auto-select-model', {
      detail: {
...
function initEndpointForm() {
  ...
  document.querySelectorAll('.adm-quickstart-section').forEach((sec) => {
    ...
    if (!head) return;
    const key = 'pantheon.addModels.' + sec.id + '.open';
    let open = false;
...
const _TOKEN_SCOPES = [
  ...
  { key: 'cookbook:read',     label: 'Forge read',     detail: 'List cookbook tasks + tail their tmux output' },
  { key: 'cookbook:launch',   label: 'Forge launch',   detail: 'Launch and stop cookbook serve tasks' },
];
  ...
  try {
    ...
    list.querySelectorAll('[data-adm-del-token]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try { window.dispatchEvent(new CustomEvent('pantheon-integrations-changed')); } catch (_) {}
      });
    });
...
function initBackup() {
  el('adm-exportDataBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const filename = match ? match[1] : 'pantheon_backup.json';
