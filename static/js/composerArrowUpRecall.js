...
export function wireArrowUpRecall(composer, getUserMessages, options = {}) {
  ...
  const debug = (...args) => {
    try {
      if (localStorage.getItem('pantheonArrowRecallDebug') === '1') {
  ...
  composer.addEventListener('input', () => {
    ...
    lastRecalledValue = '';
    recallHistory = [];
    try { delete composer.dataset.pantheonRecallIndex; } catch (_) {}
  });
  ...
  composer.addEventListener('keydown', (e) => {
    ...
    if (currentIndex < 0 && currentValue) {
      const markedIndex = Number(composer.dataset?.pantheonRecallIndex);
    ...
    if (e.key === 'ArrowDown') {
      ...
      try { composer.dataset.pantheonRecallIndex = String(nextIndex); } catch (_) {}
