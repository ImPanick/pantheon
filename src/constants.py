...
DATA_DIR = os.getenv("PANTHEON_DATA_DIR", get_default_data_dir())

# Data file paths
# Single source of truth: every persisted file/dir lives under DATA_DIR, which
# is the ONLY place PANTHEON_DATA_DIR is read. Import these constants instead of
...
# Outbound cooldowns that must survive a restart (`P15-09`). Wall-clock
# deadlines, not monotonic readings — see the note in src/rate_limiter.py.
OUTBOUND_STATE_FILE = os.path.join(DATA_DIR, "outbound_state.json")
...
MAIL_ATTACHMENTS_DIR = os.getenv("PANTHEON_MAIL_ATTACHMENTS_DIR", os.path.join(DATA_DIR, "mail-attachments"))
...
def internal_api_base() -> str:
    """Base URL for in-process loopback calls to Pantheon's own API.

    Agent tools and background jobs reach admin-gated routes by calling the
    running server over HTTP. Resolution order:
      1. PANTHEON_INTERNAL_BASE  - explicit override (e.g. behind a TLS proxy).
    ...
    """
    override = os.environ.get("PANTHEON_INTERNAL_BASE")
    if override:
