...
def _find_browser_executable() -> str:
    ...
    """
    configured = os.environ.get("PANTHEON_BROWSER_EXECUTABLE", "").strip()
    if configured:
        return configured
...
async def register_builtin_servers(mcp_manager):
    ...
    if MCP_DISABLED:
        logger.info("Built-in MCP servers disabled via PANTHEON_DISABLE_MCP")
        return
        ...
        for server_id, cfg in _BUILTIN_NPX_SERVERS.items():
            ...
            if BROWSER_MCP_REQUIRE_CACHE and pkg_spec and not await _is_npx_package_cached(npx_path, pkg_spec):
                logger.warning(
                    ...
                    f"          (run once, then restart Pantheon)\n"
                    f"  Notes:  PANTHEON_BROWSER_MCP_REQUIRE_CACHE=1 is set, "
                    f"so Pantheon will not install browser automation on startup."
                )
                continue
