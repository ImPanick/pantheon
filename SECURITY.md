...
- **pip-audit** scans `requirements.txt` and `requirements-optional.txt` against the advisory database, on pull requests and on pushes to `main`. Advisory only — it reports, it does not block, because it flags pre-existing issues that a given pull request did not introduce.
- **Trivy** scans the built container image, which sees more than `pip-audit` does: everything installed into the image, including packages that are in no `requirements*.txt`. Also advisory, and findings land in the repository's Security tab.

### One accepted risk: `basicsr` 1.4.2 (CVE-2024-27763)

Stated here rather than left for you to find in a scan.

**What ships.** `docker/build-realesrgan-wheels.sh` builds patched wheels for `basicsr==1.4.2`, `gfpgan==1.3.8` and `facexlib==0.3.0`, and the `Dockerfile` installs them with `--no-deps`. So `basicsr` 1.4.2 is present in the default Docker image whether or not you ever use Real-ESRGAN. It is not in `requirements.txt` or `requirements-optional.txt`, so `pip-audit` does not see it; Trivy does, and will report it against the image.
