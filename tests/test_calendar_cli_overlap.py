...
`pantheon-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "pantheon-calendar"
    loader = importlib.machinery.SourceFileLoader("pantheon_calendar_cli", str(path))
