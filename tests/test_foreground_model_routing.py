...
def _chat_stream_endpoint(
    ...
    *,
    ...
    context = SimpleNamespace(
        ...
        web_sources=[],
        used_memories=[],
        # `B60`. This double stands in for `ChatContext`, and the route now
        # hands the loop a single `suppress_skills` computed from it.
        skills_enabled=True,
        uploaded_files=[],
        uprefs={},
        was_compacted=False,
...
def test_direct_low_signal_fallback_estimates_winning_route_prompt(
    ...
    backup = ("https://backup.example/v1", "pantheon-qwen-backup", {})
    candidate_requests = []
...
def test_skill_activation_reaches_later_fallback_request_and_pinned_round(monkeypatch):
    ...
    assert any(
        "route=pantheon-qwen-backup; tools=grep,manage_skills" in (message.get("content") or "")
