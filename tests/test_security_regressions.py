...
def test_ollama_cookbook_runner_does_not_force_public_bind():
    ...
    assert 'OLLAMA_HOST="0.0.0.0:${PANTHEON_OLLAMA_PORT}" ollama serve' not in route
    assert 'OLLAMA_HOST="${PANTHEON_OLLAMA_HOST}:${PANTHEON_OLLAMA_PORT}" ollama serve' in route
...
def test_inprocess_pollers_gate(monkeypatch):
    """The PANTHEON_INPROCESS_POLLERS env var must let operators kill
    ...
    for off in ("0", "false", "no", "off", "FALSE", "Off"):
        monkeypatch.setenv("PANTHEON_INPROCESS_POLLERS", off)
    ...
    for on in ("1", "true", "yes", "anything-truthy"):
        monkeypatch.setenv("PANTHEON_INPROCESS_POLLERS", on)
...
def _load_search_content_for_test(monkeypatch, name="services.search.content_under_test"):
    ...
    cache.CONTENT_CACHE_DIR = Path("/tmp/pantheon-test-content-cache")
...
def test_google_mcp_oauth_uses_configured_redirect_base(monkeypatch):
    ...
    assert (
        ...
        == "https://pantheon.example/app/api/mcp/oauth/callback"
    )
